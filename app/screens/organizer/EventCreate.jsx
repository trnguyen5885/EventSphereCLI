...
const EventCreate = () => {
  ...
  const handleSave = async () => {
    ...
    if (coordinates) {
      console.log('Toạ độ:', coordinates);
      setLatitude(coordinates.latitude);
      setLongitude(coordinates.longitude);
      setLocation(fullAddress);
      // Gửi lên server, lưu vào state, v.v.
    } else {
      Alert.alert('Không thể lấy tọa độ từ địa chỉ');
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.status) {
        ...
        setBanner('');
        setImages([]);
        setTimeStart(new Date().getTime()); // hoặc null nếu bạn dùng timestamp
  ...
  <View style={styles.container}>
    ...
    <ScrollView showsVerticalScrollIndicator={false}>
      ...
      <TouchableOpacity style={styles.button} onPress={handleSave}>
        <Text style={styles.uploadButtonText}>Xác nhận địa chỉ</Text>
      </TouchableOpacity>
